...
def read_messages(state: State) -> State:
    ...
    if ultima_msg and not mensagem_mudou:
        state["repeated_count"] = state.get("repeated_count", 0) + 1
    else:
        state["repeated_count"] = 0

    state["current_message"] = ultima_msg
    state["current_options"] = ultimas_opcoes
    ...
    # (state["messages"] = [...] soma ao histórico existente, não
    # substitui) -- sem essa checagem, a mesma mensagem do bot seria
    # duplicada no histórico a cada volta do grafo, mesmo quando ele
    # não disse nada novo (ex: enquanto aguardamos uma resposta).
    if mensagem_mudou:
        state["messages"] = [f"BOT: {ultima_msg}"]
    else:
        state["messages"] = []
...
def generate_response(state: State) -> State:
    ...
    state["response_tipo"] = resultado["tipo"]
    state["response"] = resultado["valor"] or ""
    state["error"] = False

    print(
        f"[generate_response] turno={state.get('turns', 0)} "
        f"msg={state['response']!r} tipo={state['response_tipo']!r}"
    )

    return state


def send_message(state: State) -> State:
    ...
    if tipo == "aguardar":
        ...
        state["aguardar_count"] = state.get("aguardar_count", 0) + 1
        time.sleep(2)
        state["messages"] = ["USER: (aguardando o bot continuar...)"]
        state["turns"] = state.get("turns", 0) + 1
        print(f"[send_message] turno={state['turns']} aguardando (sem enviar)")
        return state

    state["aguardar_count"] = 0
    ...
    state["messages"] = [f"USER: {resposta}"]
    state["turns"] = state.get("turns", 0) + 1

    print(
        f"[send_message] turno={state['turns']} msg={resposta!r} "
...
"""
    ...
    state["summary"] = resposta.content
